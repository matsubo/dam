...
const SECTIONS: { title: string; terms: Term[] }[] = [
  {
    ...
    terms: [
      ...
      },
      {
        ...
        reading: 'すいけい',
        body:
          '一つの河川とそれに接続する支川群、湖沼などを一体としてとらえた集水域。河川法に基づき「一級水系」「二級水系」が指定される。本サイトでは国土数値情報 W07 を一次ソースとし、644 水系を収録。',
        also: '一級水系 / 二級水系 / その他',
      },
      {
  ...
  },
  {
    ...
    terms: [
      ...
      },
      {
        ...
        reading: 'りすいようりょう',
        body:
          '通常運用で利用可能な貯水容量。総貯水容量から死水容量および洪水調節容量を除いた残り。水道・かんがい・発電などの用途に充てられる。本サイトの「貯水率」の分母に用いる。',
      ...
      },
      {
        ...
        reading: 'ちょすいりょう',
        body:
          'ある時点でダムに実際に貯まっている水の量（容積）。単位 m³。観測値カラム名は storage_volume_m3。',
      ...
      },
      {
        ...
        reading: 'ちょすいりつ',
        body:
          '本サイトでは「現在の貯水量 ÷ 利水容量」で計算する。利水容量を持つダムだけが対象。100 % を超えた場合は表示上 100 % にクリップする（実運用上は洪水期の制限水位等に依存し、表示の単純化のため）。利水容量が登録されていないダムでは — と表示。',
      ...
      {SECTIONS.map((section, idx) => (
        <section key={section.title} className="mb-10" id={`section-${idx}`}>
          ...
          <dl className="space-y-5">
            {section.terms.map((t) => (
              <div key={t.id} id={t.id} className="border-l-4 border-outline-variant pl-4 scroll-mt-20">
