      ...
      {q.length === 0 ? (
        ...
        <>
          {watersheds.length > 0 && (
            <section className="mb-10">
              <h2 className="text-lg font-semibold mb-3">
                水系 <span className="text-on-surface-variant text-sm font-normal">({watersheds.length})</span>
              </h2>
              <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {watersheds.map((w) => (
                  <li key={w.slug}>
                    ...
                    >
                      <div className="font-display font-semibold text-on-surface">{w.name}</div>
          ...
          {dams.length > 0 && (
            <section className="mb-10">
              <h2 className="text-lg font-semibold mb-3">
                ダム <span className="text-on-surface-variant text-sm font-normal">({dams.length})</span>
              </h2>
              <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {dams.map((d) => (
                  <li key={d.slug}>
                    <Link href={`/dams/${d.slug}`} className="card-surface block no-underline">
                      <div className="font-display font-semibold text-on-surface">{d.name}</div>
