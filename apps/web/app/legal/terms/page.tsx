  ...
  return (
    <article className="max-w-3xl mx-auto px-5 md:px-10 py-10">
      ...
      <Section n="2" title="提供データの性質">
        <p>
          本サービスが提供するダム諸元・観測値は、国土交通省・国土数値情報・ダム便覧・
          国土地理院・気象庁等の公開情報をもとに、本サービスが集約・正規化したものです。
          原典の最新性・正確性は保証されず、また、本サービスが行う名寄せ・単位換算・補間
          などの加工を経た値が含まれます。
        </p>
        <p>
      ...
      <Section n="3" title="利用条件">
        <ul className="list-disc list-inside space-y-1">
          ...
          <li>
            API
            の利用には Google アカウントによる認証と発行されたキーが必要です。
      ...
      <Section n="4" title="レート制限">
        <p>
          ...
          上限を超えた呼び出しは HTTP 429 を返します。継続的に高負荷をかける利用 (
          DoS 的アクセス、超高速ポーリング等) は禁止します。
        </p>
      </Section>
      ...
      <Section n="6" title="知的財産・ライセンス">
        <p>
          ...
          として公開予定で、データソース部分のライセンスは原典のライセンスに従います
          (国土数値情報: 政府標準利用規約 2.0 互換、Wikipedia: CC-BY-SA、ほか)。
          本サービスの UI 上の写真は{' '}
